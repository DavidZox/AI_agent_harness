...
| 模組 | 行數 | 內容 |
|---|---|---|
| `agent.py` | 190 | `SkillAgent`：組合各 mixin；`__init__`（所有狀態的初始值，見第 5 節）與 token 計量（`count_tokens`、`context_tokens`、`_record_usage`） |
| `conversation.py` | 119 | `ConversationMixin`：`set_current_task`（任務線）、`reset_conversation`（/clear）、`ask_ai`（含空白回覆重試）、`_chat_once`、`parse_reply` |
| `prompt.py` | 151 | `PromptMixin`：`get_system_prompt` 與它的各區塊（Objective、計畫、任務錨點 `_build_task_anchor_text`）、Plan 模式的請求與核准 |
| `dispatch.py` | 227 | `DispatchMixin`：`run_tool`（兩階段揭露：技能名稱回規格、腳本路徑才執行）、`_exec_script`（subprocess、環境變數、600 秒防線）、`_load_skill_doc`、`list_skills`、狀態標記同步 |
| `tool_summary.py` | 407 | `ToolSummaryMixin`：`summarize_tool_result`、`_result_recall`，以及它們共用的 `_extract_task_oriented`、`_extract_with_followups`（自動追問）、`_validate_related`、`_followup_guidance`（下一步建議）、`_render_tool_summary` |
| `tool_use_index.py` | 119 | `ToolUseIndexMixin`：`tools_use_index.md` 的寫入（`_append_tool_use_index`、`clip_hint`）、讀取與 system prompt 尾端的顯示（`_tool_use_index_block`） |
| `archive.py` | 110 | `ArchiveMixin`：原文存檔 `_archive_tool_result`、`index.md`、保留上限 `_prune_tool_results`、`_max_archived_id`（跨 session 續編）、`result_file_path`（Web 開原文用） |
| `trajectory.py` | 175 | `TrajectoryMixin`：`_record_trajectory`（每執行一支腳本記一筆並觸發存檔）、起點、`/trajectory` 的顯示與步驟挑選 |
| `compression.py` | 375 | `CompressionMixin`：`_split_for_compression`（保留區切點）、`summarize_messages`（滾動摘要 session）、`compress_context_to_file`、背景壓縮、`ensure_context_budget`（硬水位） |
| `make_skill.py` | 550 | `MakeSkillMixin`：`/make_skill` 的草擬、驗證、產生規格與組合腳本、重播、註冊 |
| `turn.py` | 99 | CLI 與 Web 共用的回合函式：`_content_for_context`（工具回傳進主對話的三條路）、`context_kind`、`after_turn_compression`（軟水位）、`_append_discarded_tool_result` |
| `protocol.py` | 107 | 回覆協議與訊息包裝：`parse_agent_reply`、`action_text`、`tool_result_message`、`attach_skill_docs`、`is_exempt_result` |
| `schemas.py` | 119 | 四個 Ollama `format=` schema：`AGENT_REPLY_SCHEMA`、`SUMMARY_SCHEMA`、`TOOL_SUMMARY_SCHEMA`、`MAKE_SKILL_SCHEMA` |
| `config.py` | 165 | 所有常數與環境變數（第 10 節），以及 `PROJECT_ROOT`（專案根目錄，`AGENT.md`、`skills_system/`、`logs/` 都以它為準） |
| `cli.py` | 358 | CLI：`main()`（slash 指令、回合迴圈、auto／hybrid／manual 決策）、`_run_plan_flow`（規劃階段不呼叫 `run_tool`）、`_run_make_skill_flow` |

依賴方向是單向的：`config`、`schemas` 不依賴其他模組，`protocol` 只依賴 `config`；各 mixin 只依賴這三個、彼此不互相 import；`agent` 組合所有 mixin；`turn`、`cli` 在最上層。沒有循環 import。

### 4.2 `web_app/`：網頁介面（原 `web_console.py`）

| 模組 | 行數 | 內容 |
|---|---|---|
| `state.py` | 68 | 全域狀態：唯一的 `SkillAgent` 實例、模式開關 `state`、待決策的工具結果 `pending`、待核准的計畫、附件、手動載入的技能規格 |
| `flows.py` | 353 | 回合流程：`run_turn`（ask_ai → run_tool 迴圈）、`apply_decision`（manual／hybrid 的決策）、Plan 模式、技能草稿、附圖的視覺 sub-session、狀態列 `build_stats` |
| `commands.py` | 275 | slash 指令：`handle_slash_command`、「/」選單清單 `SLASH_COMMANDS`、`/menu` 的說明文字 |
| `server.py` | 366 | HTTP：`ConsoleHandler`（`/`、`/api/*`、`/static/vision/*` 路由）、NDJSON 串流 `EventStream`、`main()` |
| `static/index.html` | 652 | 網頁本身（HTML／CSS／JS 都在這一個檔案，無外部依賴） |
