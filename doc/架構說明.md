...
main session 執行 `result_recall <編號>[,<編號>] "<使用者的話>"` 時，`_result_recall()` 讀出那幾份完整原文（輸入上限平均分配給每一份），以任務線為錨點（模型給的參數只當聚焦點，避免它把清單描述抄進來當問題），走同一套擷取。輸出以 `TOOL_SUMMARY_TAG` 開頭，所以不會再被摘要一次，也不寫進清單。
