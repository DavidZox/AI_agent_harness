...
2. **搜尋工具與指令（兩段式載入 / Progressive Disclosure）**：
   - `SKILLS.md` 是輕量索引，僅列出每個技能的名稱、一行描述與規格文件連結，先由此挑選候選技能。
   - 若不確定該技能的完整參數順序、範例或異常處理方式，**先執行** `EXECUTE: view_file [規格文件連結路徑]` 讀取對應的 `skills_system/tools/<name>.md` 詳細規格。
   - 確認參數格式後，於**下一輪**再實際 `EXECUTE:` 呼叫該技能（依據「單次回答限制」規則，讀取規格與執行技能不可在同一輪輸出）。
   - 對於已經熟悉、參數單純的技能（如 `list_dir`、`view_file` 本身），可直接執行，不需每次都重新讀取規格文件。
