...
**一句話**：使用者用自然語言下指令 → 本地模型（gemma4:e4b）每輪回一個 JSON `{thought, reply, action}` → harness 只看 `action`：填技能名稱就把該技能的規格文件塞回上下文（這輪不執行），填規格裡標明的腳本路徑才真的執行 → 腳本的 stdout（`[PASS]`／`[ERROR]` 開頭）回給模型 → 直到模型不再需要工具。模型一行程式都不寫，所有能力都是 `skills_system/scripts/` 裡的既有腳本。

| 檔案 | 角色 |
|---|---|
| `Agent_Runner.py` | 唯一的核心：`SkillAgent`（對話狀態、規格載入、腳本執行、上下文壓縮、操作軌跡）＋ CLI |
| `web_console.py` | 網頁介面，重用 `SkillAgent`；標題列顯示模式、cwd、目標容器、tokens |
...
| `Memory.md`／`skills_system/memory/` | 全域常駐記憶／綁定技能的記憶（`modify_memory` 寫入） |
| `logs/` | 壓縮摘要、操作軌跡 `trajectory.jsonl`、`/make_skill` 稽核 |

**技能分四群**（見 `SKILLS.md`）：檔案系統（`list_dir`／`search_text`／`find_file`／`change_dir`／`view_file`）；容器與 ROS2（`docker_*`／`ROS2_*`，共用 `_docker_common.py`）；調度系統（`workpackage_send`／`workpackage_status`／`workpackage_cancel`／`overpending_cancel`／`semantic_map`，共用 `_rmf_common.py`，對象是 fih_rmf_system 的 web_console）；其他（`stt_engine`、`image_inspect`、`modify_memory`）。`/make_skill` 可把做對的步驟序列編譯成組合技能（`_composite.py`）。

**harness 替模型維護的狀態**（system prompt 的 Current Agent State）：工作目錄（`change_dir` 改、腳本以它為 cwd）與目標容器（`docker_open` 選定或最近一次成功操作的容器，容器技能可省略名稱、模型不再反問）。

**上下文管理**：單次工具回傳超過 500 tokens 只給模型「成功／失敗」（`[PASS][digest]` 開頭的分析型輸出放寬到 1500）；整體上下文超過軟水位在回合結束後壓成滾動摘要、超過硬水位在呼叫前壓。

**驗證方式**：`scripts/mock_server.py` 模擬 web_console 供離線測試；容器技能用假 `docker`；每次改動以 stub `ollama.chat` 的測試套件加 gemma4:e4b 真模型情境驗證，不對真系統送任何會改變狀態的請求。
...
**腳本回傳慣例與逾時機制**：所有技能腳本統一由 stdout 回傳結果，成功以 `[PASS]` 開頭、失敗（含逾時）一律以 `[ERROR]` 開頭並說明原因與建議動作——`_content_for_context()` 就是靠這個前綴判定成功/失敗，`SKILLS.md` 開頭也向模型說明了這個慣例。每支會呼叫外部程序或網路的腳本都有自己的逾時上限（各 `tools/<name>.md` 的「語法 / 參數規範」有寫）；容器相關腳本共用 `scripts/_docker_common.py`，在容器內以 coreutils `timeout` 包住指令，逾時會真正終止容器內的程序而不是只殺掉宿主機端的 `docker exec`，並把常見的 docker / ros2 錯誤翻成可行動的說明。容器內 ROS2 指令的環境由 `_docker_common.ROS2_ENV_FALLBACK` 補齊：shell 沒有 `ros2` 就 source `/opt/ros/<distro>`，再靜默 source 第一個找到的 colcon workspace overlay（自訂訊息型別才 echo 得出來），shell 沒有 `ROS_DOMAIN_ID` 時從容器內正在跑的 ROS 節點的 `/proc/<pid>/environ` 複製 `ROS_DOMAIN_ID`／RMW／CycloneDDS／discovery 變數——實測 fih_rmf_system 容器的節點跑在 domain 98，而 `bash -ic` 既沒有 ros2 也沒有 domain，改版前 `ROS2_topic_list` 會是空白、`ROS2_topic_echo` 只回「does not appear to be published yet」。`ROS2_topic_echo` 另有 `--duration 秒`：以 `timeout --preserve-status -s INT` 擷取一段時間的所有訊息（`--full-length`，輸出上限 400 KB），宿主機用 PyYAML 逐則解析、攤平成 `a.b.c` 欄位（String 型 topic 裝的 JSON 也會解開），回報則數與頻率、第一與最後一則原文、每個欄位的數值範圍／字串種類／固定不變清單，讓模型分析「一段時間的變化」而不是一筆瞬間值。腳本輸出以 `[PASS][digest]` 開頭代表「已經為模型整理過的分析摘要」（`workpackage_status --watch`、`semantic_map`、`ROS2_topic_echo --duration` 都是）：`_content_for_context()` 對它放寬到 `DIGEST_TOKEN_THRESHOLD`（1500 tokens）才精簡，CLI／Web 也不標 ⚠️，因為這類結果的價值就在內容本身，換成「指令已成功執行」模型就沒有分析依據；腳本自己要負責把摘要控制在範圍內。`SkillAgent.run_tool` 另設 `TOOL_EXEC_TIMEOUT`（600 秒）作為最後防線，並在腳本以非零 exit code 結束時補上 `[ERROR]` 前綴、以及在腳本完全沒有輸出時給出明確訊息——否則空字串會被誤判成「沒有工具需要執行」。

**目標容器（Current Agent State）**：`SkillAgent.target_container` 跟 `current_cwd` 是同一套機制。`run_tool` 以環境變數 `TARGET_CONTAINER` 傳給每支腳本，容器技能（`docker_runcmd`、`ROS2_*`）省略 `<container_name>` 時由 `_docker_common.resolve_container()` 取它，沒有就回 `[ERROR]` 提示先用 `docker_containers` 查、`docker_open` 選定；腳本成功操作某個容器後在輸出末行印 `[TARGET_CONTAINER] <名稱>`（比照 `cd` 的 `[CWD_CHANGED]`；放末行是為了不破壞 `[PASS]`／`[PASS][digest]` 的開頭判定，與目前目標相同時不印），`_sync_state_from_tool_output()` 同步，所以目標容器＝最近一次成功操作的容器，`docker_open`（與 `docker_est` 建立的新容器）是刻意選定／切換用。預設空、`/clear` 不清（跟 cwd 一樣）。system prompt 的 `CURRENT_TARGET_CONTAINER` 加上 `AGENT.md` 執行協議第 1 點讓模型直接用而不再問使用者要看哪個容器；Web Console 標題列顯示 `container:`；軌跡記錄與組合技能重播也帶著它（`_composite._sync_state` 讓前一步選定的容器接到下一步）。`docker_runcmd` 只有一個參數時整串是指令、用目標容器；有目標容器且第一個參數不是任何現有容器的完整名稱（`docker ps -a` 完全比對，不猜部分名稱）時也把整串當指令——模型常忘記引號或省略名稱。`ROS2_topic_echo`／`ROS2_node_info` 以 `/` 開頭的第一個參數判定為 topic／node 名稱、容器省略。實測 `gemma4:e4b`：已設目標時直接省略容器名稱列 topic、不再反問；要求換容器時先 `docker_open` 新名稱再列 node，目標隨之更新；未設目標時會先 `docker_containers` 列清單，但兩個容器都在跑時它自行挑了第一個，因此 `docker_containers` 規格加了「多個容器時請使用者指定」。
...
│   └── scripts/<name>_cmd.py # 各技能實際執行的腳本（容器類共用 _docker_common.py；工作包類共用 _rmf_common.py；組合技能共用 _composite.py）
...
3. **工作包技能群依賴 fih_rmf_system 的 web_console**：原本合併在一個 `workpackage_est` 技能裡的七種模式（規格約 1200 tokens、腳本 840 行）已拆成五個技能，各自的規格只在需要時載入；共用層 `scripts/_rmf_common.py`（HTTP、純標準庫迷你 WebSocket 客戶端、`/api/topology` 語義地圖與站點對應、快照查詢、`--url` 共用選項）對齊 `distribute_task.FastAPIBridgeNode` 的對外能力：
   - `workpackage_send`：`POST /api/send_tasks`（轉發到 `incoming_work_packages`）。站點可寫語意名稱（「加工線通道-6」→ a0），對不到或對到多站回 `[ERROR]`。循環對齊 orchestrtor 的 `loop_count`（None＝循環到手動刪除、正整數＝N 輪；web 介面也是「留空＝無限」）：`--loop` 不接數字＝無限，也收 `--loop 0`／`無限`／`--forever`，`--loop N`＝N 輪。實測 `gemma4:e4b` 把舊措辭「不接＝循環到取消」讀成「不加 `--loop`」，三次都送出單輪任務，且發送回傳約 570 tokens 超過 `TOOL_RESULT_TOKEN_THRESHOLD` 被精簡、看不到 `loop: 否`；現在規格用「使用者說無限循環 → 加 `--loop` 不接數字」的正面對應，回傳第一行寫出站數／循環／機器人並壓在門檻內。帶了別的技能的選項（`--status`、`--map`…）會直接指路到對應技能。`--ros2 [容器]` 備用入口不經 web_console 直接發布 topic（容器省略＝目標容器）。
   - `workpackage_status`：WebSocket `/api/ws` 快照；`--watch 秒` 持續收幀整理變化（站點推進、OverPending 進出、機器人狀態、事件），因為一幀只是瞬間。
   - `workpackage_cancel`：`DELETE /api/work_packages/{id}`，送出後讀快照確認。
   - `overpending_cancel`：`DELETE /api/overpending_tasks/{id}`，先確認任務此刻在 OverPending 逾時區才刪（distribute 的任務在 raw 等超過 `threshold_ovp_sec` 才進 OverPending，再過 `threshold_recovery_sec` 又回流 raw，不在逾時區時 DELETE 無效），不在時回報所在佇列，`--wait 秒` 等它進來再刪。
   - `semantic_map`：`GET /api/topology`（站點代號、semantics.yaml 的語意名稱與說明如 VLM 判讀紀錄、座標、路段）加一幀快照，把機器人座標／`target_id`／`current_edge`（RobotState 用節點 id，會換成代號）與 work package 當前站對到每個站點；`--stations`／`--robots` 只列清單。

   技能**不讀任何地圖檔**，全部問 web_console，所以看到的永遠是它目前場域（launch 的 `map_*` 參數，實測為 `fih_rmf_maps/isaacsim/fih_humanoid_amrScene.geojson`）。容器 `network_mode: host`，宿主機可直連 8020。`scripts/mock_server.py` 為純標準庫版本，模擬上述全部端點（含持續推幀的 WebSocket、拓譜圖、每 8 秒在 raw／OverPending 間來回的示範任務），離線測試用 `--port 8021` 啟動並以 `--url` 或 `RMF_WEB_CONSOLE_URL` 指過去；舊版 `workitem_est`／`workpackage_est` 已移除。
