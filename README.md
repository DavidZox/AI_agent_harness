...
**工具結果存檔與回查（`result_list`／`result_grep`／`result_view`）**：摘要會丟細節、原文進主對話會污染上下文，兩者都不選：每次腳本執行的完整原始輸出由 `_record_trajectory()` 順手存成 `logs/tool_results/<session>_<編號>_<腳本>.md`（簡單 key: value 檔頭：編號、時間、腳本、指令、cwd、容器、狀態、字數、當時的任務；不用 PyYAML），並在 `index.md` 記一行；摘要 session 產出的「回答」會回填到索引那一行。編號＝軌跡 id，啟動時從既有存檔的最大編號續編（`_max_archived_id`，跨 session 不重複，純數字的 `#16` 在任何一次啟動都指到同一份），`/make_skill` 與稽核對得上。技能規格文件不存（它不是執行結果）。摘要 schema 多了 `suggested_questions`（最多 3 個延伸方向，各附可搜尋的關鍵字），附註改成「細節用 `result_grep <編號> <關鍵字>` 搜，不要重跑工具」——觀察型技能不必再等一次擷取，摘要只讀頭尾時被省略的中段也搜得到。交握原則寫在規格文件與附註裡（`AGENT.md` 沒有預算，見 2026-09-26 那段）：有真的缺口（`not_covered`）時，主模型要用自然的一句話問使用者，不列清單、不照抄 `suggested_questions` 的文字；已經直接回答時，延伸方向只是順口一提或略過。使用者回答追問時最新一句常只剩關鍵字，所以摘要錨點帶「任務線」——最近 3 則使用者訊息（`task_history`）。`result_grep` 的輸出走一樣的門檻：超過 500 tokens 交給獨立 session（它拿到的正是使用者的追問），沒超過直接進主對話。安全：正則長度上限 200、逐行比對帶 3 秒預算（防災難性回溯）、路徑只取檔名鎖在存檔目錄；保留最近 200 個檔且總量 50 MB（`AGENT_TOOL_RESULTS_KEEP`／`AGENT_TOOL_RESULTS_MAX_MB`），刪最舊的並同步索引；`logs/` 已 `.gitignore`（輸出可能含敏感內容）。CLI 印「📄 已存檔 #16」，Web 卡片標題帶編號並可點開 `/api/results/<編號>` 看原文。
...
| `result_recall` | 依檢索清單取回存檔原文、交給獨立 session 依新問題提煉 | 無（`run_tool()` 內特例分派，見 2.5） | `<編號> "<問題原文>"`：system prompt 尾端「工具使用檢索清單」有一筆跟使用者這句話有關時用，清單裡已給語法可直接執行；有缺口自動再查最多 2 跳；照樣記軌跡、產生新編號、記進清單 |
