...
3. **`workpackage_est` 依賴 fih_rmf_system 的 web_console**：技能對齊 `distribute_task.FastAPIBridgeNode` 的全部對外能力——`POST /api/send_tasks` 發送 work package（轉發到 `incoming_work_packages`）、`DELETE /api/work_packages/{id}` 取消、`DELETE /api/overpending_tasks/{id}` 刪除 OverPending 任務、WebSocket `/api/ws` 狀態快照（`--status`，純標準庫的迷你 WebSocket 客戶端）、`GET /api/topology` 語義地圖（`--map`：站點代號、semantics.yaml 的語意名稱與說明如 VLM 判讀紀錄、座標、路段，並用同一幀快照把機器人座標／`target_id`／`current_edge`（RobotState 用的是節點 id，會換成代號）與 work package 當前站對到每個站點）、`/api/stations`／`/api/robots`。技能**不讀任何地圖檔**，全部問 web_console，所以看到的永遠是它目前場域（launch 的 `map_*` 參數，實測為 `fih_rmf_maps/isaacsim/fih_humanoid_amrScene.geojson`）。發送時站點可寫語意名稱（「加工線通道-6」→ a0），對不到或對到多站回 `[ERROR]`。兩個時序設計：`--status --watch 秒` 持續收幀並整理這段時間的變化（站點推進、OverPending 進出、機器人狀態、事件），因為一幀只是瞬間；`--cancel-overpending` 先確認任務此刻在 OverPending 逾時區才 DELETE（distribute 的任務在 raw 等超過 `threshold_ovp_sec` 才進 OverPending，再過 `threshold_recovery_sec` 又回流 raw，不在逾時區時 DELETE 無效），不在時回報所在佇列，加 `--wait 秒` 會等它進來再刪。容器 `network_mode: host`，宿主機可直連 8020；web_console 沒在跑時發送可改 `--ros2 <容器>`。`scripts/mock_server.py` 為純標準庫版本，模擬上述全部端點（含持續推幀的 WebSocket、拓譜圖、每 8 秒在 raw／OverPending 間來回的示範任務），離線測試用 `--port 8021` 啟動並以 `--url` 或 `RMF_WEB_CONSOLE_URL` 指過去；循環設定對齊 orchestrtor 的 `loop_count`（None＝循環到手動刪除、正整數＝N 輪；web 介面也是「留空＝無限」）：`--loop` 不接數字＝無限循環，也收 `--loop 0`／`--loop 無限`／`--forever` 等寫法，`--loop N`＝N 輪。實測 `gemma4:e4b` 把舊措辭「不接＝循環到取消」讀成「不加 `--loop`」，三次都送出單輪任務；而且發送回傳約 570 tokens 超過 `TOOL_RESULT_TOKEN_THRESHOLD`，模型只收到「指令已成功執行」、看不到 `loop: 否`，無法自我修正。現在規格改成「使用者說無限循環 → 加 `--loop` 不接數字」的正面對應並附範例，回傳第一行寫出站數／循環設定／機器人、web_console 回應摘要成 status／count，整段壓在門檻內。舊版 `workitem_est` 已移除。
