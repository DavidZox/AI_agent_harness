...
3. **`workpackage_est` 依賴 fih_rmf_system 的 web_console**：技能對齊 `distribute_task.FastAPIBridgeNode` 的全部對外能力——`POST /api/send_tasks` 發送 work package（轉發到 `incoming_work_packages`）、`DELETE /api/work_packages/{id}` 取消、`DELETE /api/overpending_tasks/{id}` 刪除 OverPending 任務、WebSocket `/api/ws` 讀取 distribute／orchestrtor 狀態快照（`--status`，內建純標準庫的迷你 WebSocket 客戶端，只收一幀）、`GET /api/stations`／`/api/robots` 查場域。容器 `network_mode: host`，宿主機可直連 8020；web_console 沒在跑時發送可改 `--ros2 <容器>` 在容器內直接 `ros2 topic pub`。橋接節點的場域參數（map_*）是 ROS 參數、沒有 API，技能不處理。`scripts/mock_server.py` 已改為純標準庫版本，模擬上述全部端點（含 WebSocket 快照，可用 `?fail=503|422` 模擬錯誤），離線測試時用 `--port 8021` 啟動並以 `--url` 或 `RMF_WEB_CONSOLE_URL` 指過去；舊版 `workitem_est`（POST `/plan_sequence`、5 個位置參數）已移除，腳本收到 5 個位置參數會提示改用新語法。
