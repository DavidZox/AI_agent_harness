...
# 用途
在容器內執行 `ros2 node info <node>`（自動補齊 ROS2 環境）。逾時 30 秒。

# 語法
`EXECUTE: scripts/ROS2_node_info_cmd.py [container_name] <node_name>`
* `container_name`：可省略＝目前的目標容器。`node_name` 需含命名空間（如 `/nav_node`），不確定先用 `ROS2_node_list` 查。
...
# 回傳
成功：`[PASS] 節點 <node> 的資訊（ros2 node info）:` + 原始輸出（操作的容器與目標不同時末行附 `[TARGET_CONTAINER] <名稱>`）。失敗：`[ERROR] 原因`。

# 異常
* 節點不存在：用 `ROS2_node_list` 確認名稱，不要猜名稱重試。
* 沒有目標容器、容器不存在／未運行、找不到 ros2、逾時：訊息會指出原因，依提示處理。
