...
# 用途
摘要沒涵蓋的細節都還在結果檔裡。使用者追問（「那 motor_rear_left 呢」「第 17 則長什麼樣」「有沒有 timeout」）時，用摘要附註或 `result_list` 給的編號在存檔裡搜，不要重新執行觀察型工具。標頭給每個檔的命中數，直接引用。搜尋結果太長時系統會依使用者的問題擷取重點。
交握原則：使用者的要求是探索型（看一下／觀察一下）時，先把摘要的「可追問」列給使用者選關鍵字；要求明確時直接搜、直接答。
...
* `--block`：命中時回整則訊息（`ROS2_topic_echo` 存檔以 `--- #序號` 分隔每則）。
* `all`：目前 session 最近 20 個存檔。
