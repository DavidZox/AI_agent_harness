...
description: 讀取指定 topic 的一筆訊息，或用 --duration 擷取一段時間：欄位變化、頻率與全部原始訊息（容器可省略＝目標容器）。
version: 1.5.0
...
# 回傳
單筆：一筆訊息的 YAML。一段時間：`[PASS] 觀察 '<topic>' N 秒：收到 M 則（約 X Hz）` + 欄位變化清單 + 全部原始訊息；沒收到訊息也是 `[PASS]` 並說明可能原因。操作的容器與目標不同時末行附 `[TARGET_CONTAINER] <名稱>`。失敗：`[ERROR] 原因`。
