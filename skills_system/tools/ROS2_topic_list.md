...
# 用途
在容器內執行 `ros2 topic list`（自動補齊 ROS2 環境與節點所在的 ROS_DOMAIN_ID）。逾時 30 秒。

# 語法
`EXECUTE: scripts/ROS2_topic_list_cmd.py [container_name] [--filter 關鍵字]...`
* `container_name`：可省略＝目前的目標容器；要指定時給完整名稱。
* `--filter 關鍵字`：只列名稱含關鍵字的 topic（不分大小寫，可重複給多個），標頭給符合的數量。問「有幾個／哪些 topic 跟 X 有關」時用它，數量直接引用標頭，不要自己數。
...
# 異常
* 沒有目標容器又沒給名稱、容器不存在／未運行、找不到 ros2：訊息會指出原因（先用 `docker_containers` 查、`docker_open` 選定）。
