...
type: Tool
title: ROS2 話題列表
description: 列出容器內目前的 topic（容器可省略＝目標容器）。
version: 1.3.0
...
# 語法
`EXECUTE: scripts/ROS2_topic_list_cmd.py [container_name]`
...
# 回傳
成功：每行一個 topic（操作的容器與目標不同時末行附 `[TARGET_CONTAINER] <名稱>`）。沒有 topic：`[PASS] ...沒有任何輸出`（不是錯誤）。失敗：`[ERROR] 原因`。
