...
# 用途
web_console `GET /api/topology`（拓譜圖 + semantics.yaml 的語意名稱、說明如 VLM 判讀紀錄、座標、路段）加一幀快照：每站標出誰在此、誰前往中、哪個 work package 派工中。不讀地圖檔，永遠是 web_console 目前載入的場域。發送任務前想確認站點名稱、或使用者問「某站是什麼／誰在那裡」時用。
...
# 範例
`EXECUTE: scripts/semantic_map_cmd.py`　`EXECUTE: scripts/semantic_map_cmd.py 加工線通道`　`EXECUTE: scripts/semantic_map_cmd.py a3`
