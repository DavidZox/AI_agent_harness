...
# 用途
把使用者明確要求記住的經驗寫入記憶：不加參數寫入全域 `Memory.md`（每輪常駐 system prompt）；加 `--skill <技能名稱>` 寫入該技能的經驗記憶，只在載入該技能規格時出現。與某技能的用法、參數、前置條件、曾發生的錯誤有關就綁技能；通用原則、技能取捨、溝通風格、專案經驗寫全域。
...
# 回傳
`[PASS]` 含寫入位置、內容與條目數；相同內容已存在時 `[PASS]` 提示未重複寫入；技能條目過多時附整併提醒。
