...
# 用途
web_console `DELETE /api/overpending_tasks/{id}` → `cancel_overpending_task`。**OverPending 會來回**：任務在 raw 等超過約 10 秒才進 OverPending，再約 10 秒又回流 raw，DELETE 只對「此刻在逾時區」的任務有效。本技能先看快照，此刻在逾時區才刪；不在時回報所在佇列（不是錯誤），加 `--wait 秒` 會等它進來立刻刪並確認消失。
...
# 範例
`EXECUTE: scripts/overpending_cancel_cmd.py "WP001::0::0::3"`
`EXECUTE: scripts/overpending_cancel_cmd.py "WP001::0::0::3" --wait 30`（等它進逾時區再刪）
